/* 
Script visualizes the principle of operation of the EyeSwitch program and saves snapshots in order. Furthermore, by pressing "z" index of the current snapshots is written to text file "indexes".
	

Colors meaning: red - too far from LED area, orange- buffer fills up, green- buffer full->press, blue- frozen mode after press, cyan- end of frozen mode.
...
int main( int argc, char** argv )
{
    ...
    boost::filesystem::create_directories( "/home/radoslaw/eyeSwitch/snapshots/"+label.str() );

    std::string path =  "/home/radoslaw/eyeSwitch/snapshots/";
    path += label.str();
    path += "/indexes.txt";
    std::ofstream myfile;
	...
	imwrite( "snapshots/" + label.str() + "/" + filename, decision_image );
	k++;
